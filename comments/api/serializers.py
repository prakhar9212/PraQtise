    ...
    class CommentCreateSerializer(ModelSerializer):
        ...
        def __str__(self,*args,**kwargs):
            ...
            if self.parent_id:
                ...
                return super(CommentCreateSerializer,self).filter(*args,**kwargs)
        ...
        def validate(self, data):
            ...
            if not model_qs.exists() or model_qs.count()!=1:
                raise ValidationError("This is not the valid content type")
            SomeModel=model_qs.first().model_class()
            obj_qs=SomeModel.objects.filter(object_id=self.object_id)
            if not obj_qs.exists() or obj_qs.counts!=1:
                raise ValidationError("This is not the valid object_id")
            return data
